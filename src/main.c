...
typedef struct _rrt_config_t {
    ...
    double stop_dist;
    double bot_at_goal_dist;
    double bot_near_goal_dist;

    double tv_stopped_threshold;
    double rv_stopped_threshold;

    double commit_obs_max_collision
} rrt_config_t;
...
typedef struct _rrtstar_t {
    ...
    rrt_environment_t *rrt_environment_last;
...
int
is_bot_at_goal (rrtstar_t *self, rrt_goal_t *latest_goal) {
    ...
    if (dist < self->config.bot_at_goal_dist){
...
// Not currently used
int is_bot_at_goal_tight (rrtstar_t *self, rrt_goal_t *latest_goal) {
    if (!self->bot_pose_last)
        return -1;
...
int
is_bot_near_goal (rrtstar_t *self, rrt_goal_t *latest_goal) {
    if (!self->bot_pose_last)
        return -1;
    if(!latest_goal){
        return -1;
    ...
    if (dist < self->config.bot_near_goal_dist){
        if(self->verbose_motion)
...
is_committed_trajectory_in_collision (rrtstar_t *self) {
    ...
    if (max_obs_max > self->config.commit_obs_max_collision) {
...
on_goal_status (const lcm_recv_buf_t *buf, const char *channel,
                    const rrt_goal_status_t *msg, void *user){
    rrtstar_t *self = (rrtstar_t*) user;
    if(self->goal_status !=NULL){
        rrt_goal_status_t_destroy(self->goal_status);
    }
    self->goal_status = rrt_goal_status_t_copy(msg);
}
...
void read_parameters(rrtstar_t *self)
{
    ...
    self->config.bot_at_goal_dist =  bot_param_get_double_or_fail (self->param,"motion_planner.rrtstar.bot_at_goal_dist");
    self->config.bot_near_goal_dist =  bot_param_get_double_or_fail (self->param,"motion_planner.rrtstar.bot_near_goal_dist");
    self->config.stop_dist =  bot_param_get_double_or_fail (self->param,"motion_planner.rrtstar.stop_dist");
    self->config.tv_stopped_threshold =  bot_param_get_double_or_fail (self->param,"motion_planner.rrtstar.tv_stopped_threshold");
    self->config.rv_stopped_threshold =  bot_param_get_double_or_fail (self->param,"motion_planner.rrtstar.rv_stopped_threshold");
    self->config.commit_obs_max_collision =  bot_param_get_double_or_fail (self->param,"motion_planner.rrtstar.commit_obs_max_collision");
}
...
rrtstar_t *rrtstar_create(gboolean sensing_only_local, gboolean trash_tree_on_wp, gboolean verbose, gboolean draw,
    ...
    rrt_goal_list_t_subscribe(self->lcm, "RRTSTAR_GOALS", on_goals, self);

    rrt_goal_status_t_subscribe(self->lcm, "TRAJECTORY_CONTROLLER_GOAL_STATUS",
                                    on_goal_status, self);
...
optmain_publish_optimal_path (rrtstar_t *self, gboolean rampup_speed,
    ...
    for (int i = 1; i < count; i++){
        ...
        //Ramp (down) speed
        if( edist < self->config.stop_dist )
            list[i].s = self->default_tv*(edist+ 0.2/self->config.stop_dist);
...
optmain_publish_optimal_path_to_old_goal (rrtstar_t *self, gboolean rampup_speed,  double *new_root, int invalid_root) {
    ...
    for (int i = 1; i < count; i++){
        ...
        //Ramp (down) speed
        if( edist < self->config.stop_dist )
            list[i].s = self->default_tv*(edist/self->config.stop_dist);
        ...
        while (opttraj_nodes_ptr && (dist < self->lazy_collision_check_distance)) {
            ...
            while (traj_from_parent_ptr && (dist < self->lazy_collision_check_distance)) {
                ...
                if (!first_state) {
                    ...
                    if ((path_res.cost < 0) || (path_res.obs_max > self->config.commit_obs_max_collision) || (path_res.obs_max) < 0) {
...
int publish_traj (rrtstar_t *self) {
    ...
    rrt_traj_t *opttraj = (rrt_traj_t *) malloc (sizeof (rrt_traj_t));

    if (num_states > 0) {

        opttraj->num_states = num_states;
        opttraj->states = (rrt_state_t *) malloc (opttraj->num_states * sizeof (rrt_state_t));
    ...
    rrt_traj_t_publish (self->lcm, "RRTSTAR_TRAJECTORY", opttraj);
    rrt_traj_t_destroy (opttraj);
...
int publish_tree (rrtstar_t *self) {
    ...
    rrt_tree_t *tree = (rrt_tree_t *) calloc (1, sizeof (rrt_tree_t));

    tree->num_nodes = g_slist_length (list_nodes);
    if (tree->num_nodes > 0) {
        tree->nodes = (rrt_node_t *) calloc (tree->num_nodes , sizeof (rrt_node_t));
        tree->traj_from_parent = (rrt_traj_t *) calloc (tree->num_nodes , sizeof (rrt_traj_t));
    }
    else {
        tree->nodes = NULL;
    ...
    for (GSList *iter = list_nodes; iter != NULL; iter = g_slist_next(iter)) {
        ...
        if (num_states_in_traj_from_parent) {
            tree->traj_from_parent[node_index].states
                = (rrt_state_t *) calloc (num_states_in_traj_from_parent , sizeof(rrt_state_t));
            int state_index = 0;
    ...
    rrt_tree_t_publish (self->lcm, "RRTSTAR_TREE", tree);

    if(tree){
        rrt_tree_t_destroy (tree);
    }
